...
int config_load(vs_config *cfg) {
  ...
  cJSON *v;
  if ((v = cJSON_GetObjectItem(json, "volume")) && cJSON_IsNumber(v)) cfg->volume = v->valuedouble;
  if ((v = cJSON_GetObjectItem(json, "crossfade")) && cJSON_IsNumber(v)) cfg->crossfade = v->valuedouble;
  if ((v = cJSON_GetObjectItem(json, "repeat")) && cJSON_IsString(v)) {
    if (strcmp(v->valuestring, "all") == 0) cfg->repeat = VS_REPEAT_ALL;
    else if (strcmp(v->valuestring, "one") == 0) cfg->repeat = VS_REPEAT_ONE;
  }
  if ((v = cJSON_GetObjectItem(json, "shuffle")) && cJSON_IsBool(v)) cfg->shuffle = cJSON_IsTrue(v);
  if ((v = cJSON_GetObjectItem(json, "music_dir")) && cJSON_IsString(v))
    snprintf(cfg->music_dir, sizeof(cfg->music_dir), "%s", v->valuestring);
  if ((v = cJSON_GetObjectItem(json, "download_dir")) && cJSON_IsString(v))
    snprintf(cfg->download_dir, sizeof(cfg->download_dir), "%s", v->valuestring);
  cJSON_Delete(json);
  return 0;
}
...
int config_save(const vs_config *cfg) {
  ...
  cJSON_AddNumberToObject(json, "crossfade", cfg->crossfade);
